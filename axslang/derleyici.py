"""Axs -> JavaScript derleyicisi (tarayici tarafi).

Cekirdek ile ayni lexer ve parser kullanilir; burada sadece AST JavaScript'e
cevrilir. Uretilen kodun tamami `async`tir: Axs'te
...
class Derleyici:
    ...
    def derle(self, kaynak, ad=None):
        ...
        basluk = "// %s - Axs derleyicisi urunudur\n" % (ad or self.dosya or "axs")
        return (basluk
                + "AXS.calistir(async function (T) {\n"
                + bildirim + govde
                + "});\n")
...
ISLER_DOSYASI = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                             "axsweb", "tarayici", "isler.json")


def _hazir_isler():
    """Tarayici calisma zamanindaki hazir islerin adlari.

    Liste axs.js'ten uretilir (axsweb/tarayici/isler.json), boylece derleyici ile
...
def derle(kaynak, dosya=None, ad=None):
    """Axs kaynagini JavaScript'e cevirir."""
