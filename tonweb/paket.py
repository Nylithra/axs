"""TON tarayici paketi: derlenmis kodu tek bir HTML dosyasina koyar.
...
from tonlang.derleyici import Derleyici
from tonlang.okuma import dosya_oku

from . import html as H

TARAYICI_KLASORU = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tarayici")
CALISMA_ZAMANI = os.path.join(TARAYICI_KLASORU, "ton.js")

EK_STIL = """
  #ton-cikti { white-space: pre-wrap; font: 14px/1.6 ui-monospace, SFMono-Regular,
    Menlo, monospace; background: var(--yumusak); padding: 1rem; border-radius: 8px;
    margin-top: 1rem; }
  #ton-cikti:empty { display: none; }
  #ton-uygulama:empty { display: none; }
"""
...
def derle(ton_dosyasi):
    """Bir TON dosyasini tarayici JavaScript'ine cevirir."""
...
def sayfa(ton_dosyasi, baslik=None, govde=None, gomulu=True, betik_adresi=None,
    ...
    icerik = ('<div id="ton-uygulama">%s</div>\n<pre id="ton-cikti"></pre>'
    ...
    if gomulu:
        ...
        betikler = ('<script src="%s"></script>\n<script src="%s"></script>'
                    % (js_adresi or "/ton.js", betik_adresi or ("/%s.ton.js" % ad)))
