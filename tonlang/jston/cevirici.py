"""JavaScript AST'sini TON kaynagina cevirir."""
...
class Cevirici:
    ...
    def isler_topla(self, deyimler):
        """is/sinif adlarini toplar: bunlar TON'da ciplak ad olarak yazilir."""
        for d in deyimler:
            t = d.get("t")
    ...
    def _deyim(self, d, girinti):
        ...
        if t == "Disari":
            ...
            return
        if t == "Modul":
            self.yaz(girinti, "# %s %s   (TON'da: use \"dosya.ton\")" % (d["tur"], d["metin"]))
            self.uyar("'%s' satiri yorum yapildi; TON'da `use` kullanilir" % d["tur"], satir)
            return
        if t == "Dondur":
            if d["deger"] is None:
        ...
        if t == "Dene":
            ...
            return
        if t == "Etiket":
            self.todo(girinti, "'%s:' etiketi TON'da yok" % d["ad"], satir)
            self.deyim(d["govde"], girinti)
            return
    ...
    def atama_yaz(self, e, girinti):
        ...
        if islec == "&&=":
            ...
            self.yaz(girinti, "end")
            return
        self.todo(girinti, "'%s' atamasi cevrilemedi (bit islemleri TON'da yok)" % islec,
                  e.get("satir"))

    def atama_hedefi(self, e):
        """Atamanin SOL tarafi: TON'da ciplak ad ya da %x%.alan yazilir."""
        if e["t"] == "Ad":
            return e["ad"]
        if e["t"] == "Bu":
            return self.bu_adi
    ...
    def secim_yaz(self, d, girinti):
        ...
        for durum, govde in d["dallar"]:
            ...
            if govde and not self.icerir(govde, "Kir"):
                self.uyar("switch dalinda 'break' yok: TON'da alt dala dusme yoktur",
                          d.get("satir"))
    ...
    def dene_yaz(self, d, girinti):
        ...
        self.yaz(girinti, "end")
        if d["sonunda"]:
            self.yaz(girinti, "# finally: TON'da yok, asagidaki satirlar her durumda calisir")
    ...
    def params_yaz(self, params, satir):
        ...
        for tur, hedef, varsayilan in params:
            if tur == "kalan":
                self.uyar("'...%s' (kalan parametreler) TON'da yok" % hedef, satir)
                adlar.append(hedef)
                continue
            if hedef["t"] != "Ad":
    ...
    def sinif_yaz(self, d, girinti, ad=None):
        ...
        self.isler.add(ad)
        if d.get("ata"):
            self.uyar("'extends' (kalitim) TON'da yok; '%s' icin elle cozulmeli" % ad,
                      d.get("satir"))
        kurucu = None
        yontemler = []
        alanlar = []
    ...
    def ifade(self, e, cagri_hedefi=False):
        ...
        if t == "Mantik":
            if e["islec"] == "??":
                self.uyar("'??' yerine 'or' yazildi: JS'te sadece null/undefined'da, "
                          "TON'da bos degerlerin hepsinde saga gecer", satir)
        ...
        if t == "Guncelle":
            ...
            return self.ifade(e["deger"])
        if t == "Yayilim":
            self.uyar("'...' (yayilim) TON'da yok", satir)
            return self.ifade(e["deger"])
        if t == "Sira":
            return self.ifade(e["ogeler"][-1])
        if t == "Ust":
            self.uyar("'super' TON'da yok", satir)
            return "null"
    ...
    def nesne(self, e):
        ...
        for tur, anahtar, deger in e["ciftler"]:
            if tur == "yayilim":
                self.uyar("nesne icinde '...' TON'da yok", e.get("satir"))
                continue
            if tur == "hesapli":
    ...
    def cagri(self, e):
        ...
        if hedef["t"] == "Ad":
            ...
            if ad == "require":
                self.uyar("require(): TON'da `use \"dosya.ton\"` kullanilir",
                          e.get("satir"))
                return "null"
    ...
    def yerinde_yontem(self, ad, nesne, e, args):
        """JS'te reverse()/sort() listeyi YERINDE degistirir; TON'da yeni liste doner.
    ...
    def sirala_argumani(self, e):
        """sort((a,b) => a-b) gibi yaygin karsilastiricilari TON'a cevirir."""
        args = e["args"]
        if not args:
            return ""
        k = args[0]
        if k.get("t") != "Is" or len(k.get("params", [])) != 2:
            self.uyar("sort() karsilastiricisi cevrilemedi; TON'da alan adi ya da "
    ...
    def yeni(self, e):
        ...
        if hedef["t"] == "Ad":
            ...
            if ad in ("Map", "Set", "Object"):
                self.uyar("new %s() yerine TON haritasi/listesi kullanildi" % ad,
                          e.get("satir"))
    ...
    def ikili(self, e):
        ...
        if islec in BIT_ISLECLERI:
            self.uyar("bit islemi '%s' TON'da yok" % islec, e.get("satir"))
            return "0  # TODO: %s islemi" % islec
        if islec == "instanceof":
            self.uyar("'instanceof' TON'da yok; type() ile karsilastir", e.get("satir"))
            return "false"
        if islec == "in":
    ...
    def tekli(self, e):
        ...
        if islec == "+":
            return "sayi(%s)" % deger
        if islec == "typeof":
            self.uyar("typeof: TON'da tur adlari farklidir "
                      "(metin/sayi/bool/liste/harita/null)", e.get("satir"))
            return "type(%s)" % deger
        if islec == "await":
            return deger              # TON'da her sey zaten sirayla akar
        if islec == "void":
            return "null"
        if islec == "delete":
        ...
        if islec == "~":
            self.uyar("bit islemi '~' TON'da yok", e.get("satir"))
            return "0"
...
def ton_metin(s):
    """Python metnini TON metin sabitine cevirir."""
...
def cevir(kaynak, dosya=None):
    """JavaScript kaynagini TON kaynagina cevirir."""
